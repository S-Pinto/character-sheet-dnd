document.addEventListener('DOMContentLoaded', () => {
    ...
    // --- DATI DEL PERSONAGGIO (Completi e fedeli al PDF) ---
    let characterData = {
        name: "Valenor Lightbringer",
        classLevel: "Paladino 5 / Warlock 1",
        background: "Soldato",
        race: "Aasimar",
        ...
        skills: {
            athletics: { proficient: true }, acrobatics: { proficient: false }, sleightOfHand: { proficient: false }, stealth: { proficient: false },
            arcana: { proficient: false }, history: { proficient: false }, investigation: { proficient: false }, nature: { proficient: false }, religion: { proficient: false },
            animalHandling: { proficient: false }, insight: { proficient: true }, medicine: { proficient: false }, perception: { proficient: false }, survival: { proficient: false },
            deception: { proficient: false }, intimidation: { proficient: true }, performance: { proficient: false }, persuasion: { proficient: true }
        },
        proficiencies: {
        ...
        attacks: [
            { name: "Egen-Vath", bonus: "+7", damage: "2d6s + 1d4f + 4", notes: "Se l'attacco manca, l'avversario subisce comunque 3 danni da fuoco." },
            { name: "Spada Lunga (+1)", bonus: "+7", damage: "1d8+6 Tagliente", notes: "Stile Duellare (+2 danni) attivo." },
            { name: "Eldritch Blast", bonus: "+5", damage: "2d10 Forza", notes: "Gittata 18m, due raggi." }
        ],
        equipment: [
        ...
        personality: {
            appearance: "Aasimar alto e imponente, con capelli bianco-argento e occhi penetranti. Indossa un'armatura ornata bianca e oro.",
            backstory: "Ex-caporale degli Evenswords, ha stretto un patto con un'entità celestiale dopo aver perso la memoria e le sue certezze, guidato da visioni di un drago argentato."
        },
        features: [
            { name: "Attaccante Selvaggio", description: "Una volta per turno, quando colpisci con un'arma, puoi ritirare i dadi di danno e usare il risultato migliore." },
            ...
            { name: "Percezione Divina", description: "Come azione bonus, percepisci Celestiali, Immondi e Non-morti entro 18m." },
            { name: "Imposizione delle Mani", description: "Hai una riserva di 25 Punti Vita (5 x liv. Paladino) per curare." },
            { name: "Resistenza Celestiale", description: "Resistenza ai danni necrotici e radianti."}
        ],
        spells: {
            ability: "Carisma",
            saveDC: 13,
    ...
    function renderHeader() {
        ...
        document.getElementById('portrait-container').innerHTML = `
            <div class="view-item"><img id="portrait-img" src="${d.imageUrl}" alt="Ritratto del Personaggio" onerror="this.style.display='none'"></div>
            <div class="edit-item">
                <img id="portrait-img-edit" src="${d.imageUrl}" alt="Ritratto del Personaggio" onerror="this.style.display='none'">
                <label>URL o nome file immagine</label>
                <input type="text" data-path="imageUrl" value="${d.imageUrl}">
            </div>`;
        
        document.getElementById('character-name-display').innerHTML = `
            <span class="view-item">${d.name}</span>
            <input type="text" class="edit-item" data-path="name" value="${d.name}">`;

        document.getElementById('details-grid').innerHTML = `
            <div class="detail-box">
                <div class="view-item">${d.classLevel}</div>
                <input type="text" class="edit-item" data-path="classLevel" value="${d.classLevel}">
                <label>Classe & Livello</label>
            </div>
            <div class="detail-box">
                <div class="view-item">${d.race}</div>
                <input type="text" class="edit-item" data-path="race" value="${d.race}">
                <label>Specie</label>
            </div>
            <div class="detail-box">
                <div class="view-item">${d.background}</div>
                <input type="text" class="edit-item" data-path="background" value="${d.background}">
                <label>Background</label>
            </div>
            <div class="detail-box">
                <div class="view-item">${d.alignment}</div>
                <input type="text" class="edit-item" data-path="alignment" value="${d.alignment}">
                <label>Allineamento</label>
            </div>`;
    }

    function renderAbilities() {
        const container = document.getElementById('abilities-horizontal-container');
        container.innerHTML = '';
        Object.keys(characterData.abilities).forEach(key => {
            ...
            if (SKILL_MAP[key]) {
                SKILL_MAP[key].forEach(skillKey => {
                    ...
                    skillsHTML += `
                        <li>
                            <input type="checkbox" class="skill-prof edit-item" data-skill="${skillKey}" ${isProficient ? 'checked' : ''}>
                            <span class="skill-name">${skillKey.charAt(0).toUpperCase() + skillKey.slice(1)}</span>
                            <strong>${skillBonus >= 0 ? '+' : ''}${skillBonus}</strong>
                        </li>`;
                });
            }
            skillsHTML += '</ul>';
            ...
            container.innerHTML += `
                <div class="ability-box">
                    <div class="ability-header">
                        ...
                        <div class="ability-score view-item">${score}</div>
                        <input type="number" class="ability-score edit-item" data-path="abilities.${key}" value="${score}">
    ...
    function renderProficiencies() {
        ...
        document.getElementById('proficiencies-box').innerHTML = `<h2>Competenze</h2>
            <h4>Armature</h4><p class="view-item">${c.armor.replace(/\n/g, '<br>')}</p><textarea class="edit-item" data-path="proficiencies.armor">${c.armor}</textarea>
            <h4>Armi</h4><p class="view-item">${c.weapons.replace(/\n/g, '<br>')}</p><textarea class="edit-item" data-path="proficiencies.weapons">${c.weapons}</textarea>
            <h4>Strumenti</h4><p class="view-item">${c.tools.replace(/\n/g, '<br>')}</p><textarea class="edit-item" data-path="proficiencies.tools">${c.tools}</textarea>
            <h4>Linguaggi</h4><p class="view-item">${c.languages.replace(/\n/g, '<br>')}</p><textarea class="edit-item" data-path="proficiencies.languages">${c.languages}</textarea>`;
    }
    ...
    function renderCombatStats() {
        ...
        document.getElementById('hp-box').innerHTML = `<h4>Punti Vita</h4>
            <div class="hp-grid">
                <div><span class="stat-value view-item">${characterData.hp.max}</span><input type="number" class="stat-value edit-item" data-path="hp.max" value="${characterData.hp.max}"><span class="stat-label">Massimi</span></div>
                <div><span class="current-hp">${characterData.hp.current}</span><span class="stat-label">Attuali</span></div>
                <div><span class="stat-value view-item">${characterData.hp.temp}</span><input type="number" class="stat-value edit-item" data-path="hp.temp" value="${characterData.hp.temp}"><span class="stat-label">Temporanei</span></div>
            </div>
    ...
    function renderAttacks() {
        ...
        characterData.attacks.forEach((attack, index) => {
            attacksHTML += `<div class="attack-card">
                    <div class="view-item">
                        <h4>${attack.name}</h4>
                        <div class="attack-stats"><span><strong>Bonus:</strong> ${attack.bonus}</span><span><strong>Danno:</strong> ${attack.damage}</span></div>
                        <div class="attack-notes">${attack.notes}</div>
                    </div>
                    <div class="edit-item">
                        <input type="text" data-path="attacks.${index}.name" value="${attack.name}" placeholder="Nome Attacco">
                        <div style="display: flex; gap: 1rem; margin: 0.5rem 0;">
                            <input type="text" data-path="attacks.${index}.bonus" value="${attack.bonus}" placeholder="Bonus">
                            <input type="text" data-path="attacks.${index}.damage" value="${attack.damage}" placeholder="Danno">
                        </div>
                        <textarea data-path="attacks.${index}.notes">${attack.notes}</textarea>
                        <button class="delete-btn" data-type="attacks" data-index="${index}">X</button>
                    </div></div>`;
        });
    ...
    function renderEquipment() {
        ...
        characterData.equipment.forEach((item, index) => {
            equipmentHTML += `<li>
                <div class="view-item">${item.name} (${item.quantity})</div>
                <div class="edit-item" style="display:flex; gap: 5px; width: 100%;">
                    <input type="text" data-path="equipment.${index}.name" value="${item.name}" placeholder="Oggetto">
                    <input type="number" data-path="equipment.${index}.quantity" value="${item.quantity}" style="flex-basis: 70px;">
                    <button class="delete-btn" data-type="equipment" data-index="${index}">X</button>
                </div></li>`;
        });
        ...
        Object.keys(characterData.coins).forEach(coin => {
            coinsHTML += `<div><span class="stat-label">${coin.toUpperCase()}</span>
                <div class="stat-value view-item">${characterData.coins[coin]}</div>
                <input type="number" class="stat-value edit-item" data-path="coins.${coin}" value="${characterData.coins[coin]}"></div>`;
        });
    ...
    function renderFeatures() {
        ...
        characterData.features.forEach((feat, index) => {
            featuresHTML += `<div class="feature-item">
                <div class="view-item"><h4>${feat.name}</h4><p>${feat.description}</p></div>
                <div class="edit-item">
                    <input type="text" data-path="features.${index}.name" value="${feat.name}" placeholder="Nome Privilegio">
                    <textarea data-path="features.${index}.description">${feat.description}</textarea>
                    <button class="delete-btn" data-type="features" data-index="${index}">X</button>
                </div></div>`;
        });
    ...
    function renderSpells() {
        ...
        s.list.forEach((spell, index) => {
            spellsHTML += `<li>
                <div class="view-item"><strong>${spell.name}</strong> (Lvl ${spell.level})</div>
                <div class="edit-item" style="display:flex; gap:5px; width: 100%;">
    ...
    function handleInteraction(e) {
        ...
        if (target.matches('.add-btn')) {
            ...
            if (type === 'attacks') characterData.attacks.push({ name: "Nuovo Attacco", bonus: "+0", damage: "1d4", notes: "" });
            if (type === 'equipment') characterData.equipment.push({ name: "Nuovo Oggetto", quantity: 1 });
            if (type === 'features') characterData.features.push({ name: "Nuovo Privilegio", description: "Descrizione..." });
            if (type === 'spells.list') characterData.spells.list.push({ level: 1, name: "Nuovo Incantesimo" });
            renderSheet();
        }
        if (target.matches('.delete-btn')) {
            const type = target.dataset.type; const index = parseInt(target.dataset.index, 10);
            const list = type.split('.').reduce((o, i) => o[i], characterData); list.splice(index, 1);
            renderSheet();
        }
        if (target.matches('.tracker-dot')) {
            const type = target.dataset.type; const index = parseInt(target.dataset.index, 10);
            if (type === 'hd') {
                characterData.hitDice.used = (characterData.hitDice.used === index + 1) ? index : index + 1;
                renderCombatStats();
            }
            if (type === 'spell') {
                const level = target.dataset.level;
                characterData.spells.slots[level].used = (characterData.spells.slots[level].used === index + 1) ? index : index + 1;
                renderSpells();
            }
        }
        if (target.matches('[data-type^="ds-"]')) {
            const type = target.dataset.type === 'ds-success' ? 'successes' : 'failures';
        ...
        if(target.id === 'heal-btn' || target.id === 'damage-btn') {
            ...
            characterData.hp.current = isHealing ? Math.min(characterData.hp.max, characterData.hp.current + val) : Math.max(0, characterData.hp.current - val);
            renderCombatStats();
        }
    }

    // --- GESTIONE MODALITÀ E SALVATAGGIO ---
    ...
    function saveData() {
        ...
        document.querySelectorAll('.skill-prof.edit-item').forEach(el => {
            characterData.skills[el.dataset.skill].proficient = el.checked;
        });
    ...
    function loadData() {
        ...
        if (savedData) {
            ...
            // Merge per prevenire errori futuri
            Object.keys(characterData).forEach(key => {
                if (loadedData[key] !== undefined) {
                    if (typeof characterData[key] === 'object' && !Array.isArray(characterData[key]) && characterData[key] !== null) {
                        // Unisci oggetti nidificati, ma non sovrascrivere se la chiave non esiste nel caricamento
                         Object.assign(characterData[key], loadedData[key]);
                    } else {
    ...
    // Listener unificato per le interazioni
    document.body.addEventListener('click', handleInteraction);
    document.body.addEventListener('change', (e) => { // Per le checkbox
        if(isEditMode && e.target.matches('.skill-prof')) {
            const skillKey = e.target.dataset.skill;
            characterData.skills[skillKey].proficient = e.target.checked;
            renderAbilities();
        }
    ...
    document.body.addEventListener('input', (e) => { // Per aggiornamenti live dei punteggi
