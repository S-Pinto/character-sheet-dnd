document.addEventListener("DOMContentLoaded", () => {
  let characterData = {
    // ... (tutti i dati precedenti rimangono invariati)
    spells: {
      ability: "Carisma",
      spellModifier: 2,
      saveDC: 13,
  ...
  function renderSpells() {
    ...
    Object.entries(s.slots).forEach(([level, data]) => {
      let slotHTML = `<div class="spell-slot-row"><strong class="slot-label">Livello ${level}</strong><div class="tracker-grid">`;
    ...
    s.customTrackers.forEach((tracker, index) => {
      let trackerHTML = `<div class="spell-slot-row"><strong class="slot-label view-item">${tracker.name}</strong><input class="slot-label edit-item" data-path="spells.customTrackers.${index}.name" value="${tracker.name}"><div class="tracker-grid">`;
      ...
      customTrackersContainer.innerHTML +=
        trackerHTML +
        `</div><button class="delete-btn edit-item" data-type="spells.customTrackers" data-index="${index}">X</button></div>`;
    });
  ...
  // ... (tutte le altre funzioni rimangono invariate, incollo per sicurezza)
  function saveData() {
    document.querySelectorAll("[data-path]").forEach((el) => {
      ...
      if (
        path[0] === "hitDice" ||
        (path[0] === "spells" && path[1] === "customTrackers")
      )
        return;
      const value =
