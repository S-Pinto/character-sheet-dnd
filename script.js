document.addEventListener("DOMContentLoaded", () => {
  ...
  let characterData = {
    ...
    hitDice: { total: 5, type: "d10", used: 0 },
  ...
  function renderCombatStats() {
    ...
    let hdEditHTML = `<div class="edit-item edit-item-controls"><div class="control-group"><label>Num. Dadi</label><div><button class="btn btn-small" data-hd-total-change="-1">-</button><span style="padding: 0 10px; font-weight: bold;">${hd.total}</span><button class="btn btn-small" data-hd-total-change="1">+</button></div></div><div class="control-group"><label>Tipo Dado</label><div class="die-type-buttons">${dieButtonsHTML}</div></div></div>`;
    let hdHTML = `<h4>Dadi Vita</h4><div class="view-item" style="text-align:center; color: var(--c-label); margin-bottom: 0.5rem;">Lancia 1${hd.type}</div>${hdEditHTML}<div class="tracker-grid">`;
  ...
  function handleInteraction(e) {
    ...
    if (isEditMode && target.matches("[data-hd-total-change]")) {
      ...
      characterData.hitDice.total = Math.max(
        0,
        characterData.hitDice.total + change
      );
      renderCombatStats();
    }
  ...
  // --- GESTIONE SALVATAGGIO, CARICAMENTO E MODALITÀ ---
  function saveData() {
